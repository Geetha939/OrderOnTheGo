...
mongoose.connect('mongodb://localhost:27017/foodDelivery',{
    ...
    app.put('/update-user/:id', async (req, res) => {
        ...
        try {
            ...
            if (typeof usertype === 'string') user.usertype = usertype;
            if (typeof approval === 'string') user.approval = approval;
        ...
        try {
            ...
            if (user.usertype === 'restaurant') {
                ...
                if (restaurant) {
                    ...
                    await Orders.deleteMany({ restaurantId });
                    await Cart.deleteMany({ restaurantId });
    ...
    app.post('/add-new-product', async(req, res)=>{
        const {restaurantId, productName, productDescription, productCategory, productMenuCategory, productNewCategory, productPrice, productDiscount, productMainImg: productMainImgBase64 } = req.body;
        let productMainImg = req.body.productMainImg || '';
        // If client sent a base64 data URL, decode and save as file
        if (productMainImgBase64) {
            try{
                const matches = productMainImgBase64.match(/^data:(.+);base64,(.+)$/);
                let b64data = productMainImgBase64;
                let ext = '.png';
                if (matches) {
                ...
                productMainImg = `http://localhost:${PORT}/uploads/${filename}`;
            }catch(e){
